...
export const getDatabaseConfig = (type: DatabaseEnum): MysqlConnectionOptions => {
  const entityPath = path.join(environment.MAIN_FOLDER_PATH, `./src/repositories/schema/**/*.{js,ts}`);
  const migrations = path.join(environment.MAIN_FOLDER_PATH, `./src/repositories/migrations/**/*.{js,ts}`);
