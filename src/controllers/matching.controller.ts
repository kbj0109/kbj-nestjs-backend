...
export class MatchingController {
  ...
  @UserAuthGuard()
  @Get()
  async ReadMany(@Query() query: ListInput, @CurrentUser() user: JwtType): Promise<MatchingsOutput> {
    ...
    const { list, totalCount } = await this.matchingService.readManyAndTotalCountWithUser(user.userId, { skip, take });
